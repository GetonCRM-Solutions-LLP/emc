    ...
    connectedCallback(){
      ...
      accountMonthList({
        ...
        if (data) {
          ...
          this.monthoptions = [{label:'Month',value:'Month'}].concat(monthoption);
        }
      });
    ...
    @api getmilegeslist(data){
     ...
     for(let index  = 0 ; index < acctdata.length ; index++){
           ...
           accdata.push({Id : acctdata[index].Id , Date:dateformate ,IsSelected : false, checkinicon : false ,TrackingTolltip : this.checkinTolltip, Status : acctdata[index].Trip_Status__c , rowSelected:"tbody-header" ,iconcolor:"checkin_icon",timeColor:'start_time',
                         ...
                         approverejecticon : null,approverejecticon1:null,
      ...
      if(this.MilegesData.length > 0){
        ...
        if (this.approveRejectData.length == 2) {
          ...
          if(count > 0){
            ...
            modaltxt = 'You approved mileage after the ' + month + ' reimbursement report was run. This mileage will be applied to the next reimbursement period report';
          }else{
    ...
    handlemodalYes(){
      ...
      if(this.updatetext == "Not Approved Yet"){
        ...
        .then((data) => {
          ...
          makeDate.setMonth(makeDate.getMonth()-1);
          let lastmonth = makeDate.toLocaleString('default', { month: 	'long' });
          this.template.querySelector('c-dropdown-select[data-id="month"]').selectedValue = lastmonth;
          this.template.querySelector('c-dropdown-select[data-id="Types"]').selectedValue = 'All Types';
          this.template.querySelector('c-dropdown-select[data-id="Status"]').selectedValue = 'All Status';
          this.template.querySelector('c-dropdown-select[data-id="Drivers"]').selectedValue = 'All Active Users';
            ...
            for (i = 0; i < j; i++) {
              if (checkbox[i].checked == true) {
                ...
                exportTripData.push({Driver:this.MilegesData[i].DriverName , Email:this.MilegesData[i].EmailID,Status:this.MilegesData[i].Status,TripDate:this.MilegesData[i].Date,
                                    ...
                                    Drivetime:this.MilegesData[i].DriveTime,totaltime:this.MilegesData[i].DriveTime,Activity:this.MilegesData[i].Activity,
                                    trackingmethod:this.MilegesData[i].TrackingStyle,Mileges:this.MilegesData[i].Mileges,
                                    from: this.MilegesData[i].From ,
                                    Triporigion:this.MilegesData[i].Triporigion,
                                    to:this.MilegesData[i].To,
                                    Toorigion:this.MilegesData[i].Toorigion,
                                    state:this.MilegesData[i].State,
                                    tags:this.MilegesData[i].Tags,
                                    notes:this.MilegesData[i].Notes,
                                    notes:this.MilegesData[i].Notes});
              }
            }
          } 
